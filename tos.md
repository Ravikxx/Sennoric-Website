...
- A Free account includes an account-wide weekly usage allowance and a smaller allowance per rolling five-hour window, with up to three active API keys. Both allowances are lazy-start: a period doesn't begin, or count down toward reset, until your account's first chargeable request after the previous one has fully elapsed.
- Pro costs $7 per month through Stripe and includes 20x Free's weekly and five-hour usage allowances, with unlimited active API keys.
- Included usage is measured from input and output tokens, weighted to reflect each model's real compute cost — output tokens weigh more heavily than input tokens.
- Redeemable API credits are used after the included allowance. Credit balances are service accounting units, have no cash value, and cannot be withdrawn. Variable-value administrative credit codes may be redeemed in $0.001 increments.
